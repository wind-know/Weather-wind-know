...
public class MyView extends View {
    ...
    public MyView(Context context, AttributeSet attrs) {
        ...
        int mainColor = ContextCompat.getColor(context, R.color.main);
        textPaint.setColor(mainColor);
        Typeface typeface = ResourcesCompat.getFont(context, R.font.shuhei);
    ...
    public void newchange(String wen){
        float b = Float.parseFloat(wen);
        change(b/50*300);
    }
    private void change(final float trueAngle) {
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                // 计算当前比例应该的多少分
                score = (targetAngle / 300 * 50);
                score = Float.parseFloat(String.format(Locale.US, "%.1f", score));
                // 计算出当前所占比例，应该增长多少
    ...
    private void drawWaterView(Canvas canvas) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            // 对于 API 28 及以上版本，使用 INTERSECT
            canvas.clipPath(path, Region.Op.INTERSECT);
        } else {
            // 对于较低版本，可以使用 REPLACE（如果需要）
...
//         在小圆圈的外围画一个白色圈
        ...
        // 画固定值分
//        canvas.drawText("°C", radius + clipRadius/2, radius -clipRadius/4, textPaint);
        canvas.drawText("°C", radius + clipRadius / 2 + clipRadius / 10, radius - clipRadius / 4 - clipRadius / 8, textPaint);
        textPaint.setTextSize(clipRadius/6);
        // 画固定值立即优化
        canvas.drawText("点击更新", radius, radius + clipRadius/2, textPaint);
