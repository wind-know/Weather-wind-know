...
import com.example.weatherwindknow.myMap.help.DisplayUtil ;
import com.example.weatherwindknow.myMap.help.HourlyResponse;
import com.example.weatherwindknow.myMap.help.IconUtils ;
...
public class HourlyForecastView extends View implements ScrollWatcher {
    ...
    private List<WeatherHourlyBean.HourlyBean> hourlyWeatherList1;
...
//        setLayerType(View.LAYER_TYPE_SOFTWARE, null);//关闭硬件加速
        ...
        //折线颜色
        foldLinePaint.setColor(mContext.getResources().getColor(R.color.purple));
...
//    }
    public void setScrollOffset(int offset, int maxScrollOffset) {
        if (maxScrollOffset <= 0) {
            // 处理maxScrollOffset为非正数的情况
            this.maxScrollOffset = 1; // 举例：设置一个非零的默认值
        } else {
            this.maxScrollOffset = maxScrollOffset + DisplayUtil.dp2px(mContext, 50);
        }
        scrollOffset = offset;
    ...
    private int getScrollBarX() {
        if (maxScrollOffset <= 0) { // 确保maxScrollOffset不为0
            // 可以设置一个默认值或者其他逻辑来处理这种情况
            maxScrollOffset = 1; // 举例：设置一个非零的默认值
        }
